...
function parseData(
    data, oneDayData, twoDayData, oneWeekData, ethPrice, oneDayBlock) {
  // get volume changes
  ...
  data.liquidityChangeUSD = getPercentChange(data.reserveUSD,
...
const getHourlyRateData = async (pairAddress, startTime, latestBlock) => {
  try {
    ...
    const result = await splitQuery(HOURLY_PAIR_RATES, client, [pairAddress],
        blocks, 100);
